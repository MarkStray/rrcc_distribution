...
static BleManager*shareBleManager = nil;
...
+(BleManager*)shareInstance{
    @synchronized(self){
        if (shareBleManager == nil){
            shareBleManager = [[self alloc]init];
        }
    }
    return shareBleManager;
}
...
-(void)centralManagerDidUpdateState:(CBCentralManager *)central{
    ...
    if (central.state - CBCentralManagerStatePoweredOff){
        [[[UIAlertView alloc] initWithTitle:@"请打开蓝牙打印机!" message: @"" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil,nil] show];
    }else{
...
//        CBPeripheral *p = [peripheralsArray objectAtIndex:i];
//        if ([p isEqual:peripheral]){
//            [peripheralsArray replaceObjectAtIndex:i withObject:peripheral];
//            replace = YES;
//        }
//    }
//    if (!replace){
//        NSString *strName = [peripheral.name  substringToIndex:2];
//        
//        if ([strName isEqualToString:@"QS"]){
...
//    }
    ...
    if (!isExist) {
        ...
        NSString *nameStr = [peripheral.name substringToIndex:2];
        if ([nameStr isEqualToString:@"QS"]){
            l_per.m_peripheral = peripheral;
            l_per.m_peripheralIdentifier =  [peripheral.identifier UUIDString];
            l_per.m_peripheralLocaName = peripheral.name;
            l_per.m_peripheralRSSI = RSSI;
            l_per.m_peripheralUUID = peripheral.identifier.UUIDString;
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    //这个匹配的UUID 是 读出来的，，厂家已经预设好了固定特征值了
    for (CBCharacteristic *characteristic in service.characteristics){
        NSString *stringUUId = [NSString stringWithFormat:@"%@",characteristic.UUID];
        if ([stringUUId isEqualToString:@"BEF8D6C9-9C21-4C9E-B632-BD58C1009F9F"]){
...
-(void)disConnectPeripheral{
    if (m_peripheral){
        [m_manger cancelPeripheralConnection:m_peripheral];
    }else{
        [[Utility Share] alertTitle:@"没有设备连接!"];
    }
}
